...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verifica coletores
    $sql_coletor = "SELECT * FROM tb_coletores WHERE email = '{$username}' AND senha = '{$senha_md5}'";
    $res_coletor = $conexao->query($sql_coletor);

    if ($res_coletor && $res_coletor->num_rows > 0) {
        $user = $res_coletor->fetch_assoc();
        $_SESSION['usuario_id'] = $user['id_coletor'];
        $_SESSION['usuario_nome'] = $user['nome'];
        $_SESSION['tipo_usuario'] = 'coletor';
        header('location: ../Historico/Empresa/Coletor/coletas_coletor.php');
        exit();
    }
